...
Rules out: delimited strings re-split at read time, text-derived (hashed) IDs, LLM-paraphrased CellFacts surfaced as if they were the original. The fork below must satisfy the above.

- How is content stored — table-native (Rows × Columns × cells, matching the author view), SPO triples (one row per atomic CellFact), a graph, or a hybrid? This shapes the DB, API, authoring UI, and how the engine's operators (`Row | Column → Value`) get implemented.
...
- What response data do we keep per student, for how long, and at what granularity (per CellFact, per prompt, per session)?
