...
> A walking tour of what becomes possible when knowledge is stored as atomic Facts in well-formed tables, every Fact has a stable identity, and every wrong answer the student picks is a real Fact from elsewhere in the dataset. Each capability below is paired with a worked example using actual demo content from `docs/cissp/`.

## The substrate (read this first)

Three properties carry everything that follows:

1. **Atomicity.** Each cell value is one independently addressable Fact. No "and"-joined compounds. (`docs/cissp/knowledge-map.md` § Atomicity Rules.)
2. **Traceability.** Every question's prompt, correct answer, and distractor maps back to specific Fact IDs.
3. **Per-Fact response history.** We know which Facts each student saw, got right, got wrong, and *which Fact they confused it with*.

**Deterministic naming, no LLMs.** Every prompt is a coordinate (`Row | Column → ?` style). Every answer option is literal cell text — never paraphrased. The engine joins, splits, and selects Facts; it never *writes*. This is the correctness-by-construction invariant. Examples in this doc use that exact notation.

---

## 1. Chunking — one fact per option, or many

The same cell of data can yield two very different question shapes, just by changing how Facts are bundled into answer options.

**Demo source:** `docs/cissp/01-security-and-risk-management/bcp-phases.md`, Phase 5 (Develop ISCP) → Key Activity cell holds three Facts: `Document procedures`, `Document roles`, `Document responsibilities`.

**Shape A — one Fact per option (Select All):**
...
> - b) Document procedures, Determine controls, Document responsibilities ✗ *(one Fact swapped from Phase 3)*
> - c) Document procedures, Document roles, Schedule plan reviews ✗ *(one Fact swapped from Phase 7)*
> - d) Identify critical systems, Define roles, Document responsibilities ✗ *(two Facts swapped — Phase 2 and Phase 1)*
...
The engine's notion of order is just the integer in the first column. Every ordered question type is built on that one Fact.
...
This is the one only possible because every wrong answer is itself a Fact with an ID.

**Demo source:** `docs/cissp/04-communication-and-network-security/tcp-udp-sctp.md`. Student previously answered `TCP | ordering → ?` and picked `Unordered` — UDP's Value, Fact ID `tcp-udp-sctp:UDP:ordering`.
...
**Cohort variant:** if the student is a new user with no history, the engine substitutes "Facts most commonly chosen as distractors by other students for this prompt" — crowd-sourced difficulty calibration.

**Inverse-difficulty variant:** the student has correctly answered 2 of the 3 Facts in `Phase 5 | Key Activity` across past sessions (`Document procedures` ✓, `Document roles` ✓) but never `Document responsibilities`. The engine generates a fill-in for `Phase 5 | Key Activity → ?` and *only accepts* `Document responsibilities` — the gap Fact. Cell-completion mode targets the missing Fact directly.

---

## 5. Misconception surfacing — "you missed *this specific* Fact"
...
> ✗ Incorrect.
> Picked: `Frame`
> Real Fact: `Frame` belongs to `Data Link | PDU`, not `Network | PDU`.
> Correct Fact: `Network | PDU → Packet`.
>
> ⚠ Confusion pair: `Frame ↔ Packet`. Swap count: 3. Adding both Facts to review queue, paired.

The "3" count is per-Fact response history at work. The pairing in the review queue is misconception-aware spaced repetition — those two specific Facts will be co-tested until the student stops swapping them. The feedback uses the same coordinate grammar as the prompts (`Row | Column → Value`) — no English description of the data is ever generated.
...
**Phantom-fact detection:** if the student's most-picked wrong answers include strings that don't exist in *any* Fact in the dataset (their brain invented them), the engine flags it. That's a different failure mode from confusion — it's "no model at all," and the move is to drop back to easier scaffolding, not to drill.

---

## 6. Structural reasoning — questions about the table itself

Atomic, addressable Facts mean we can ask questions about the table's *shape*, not just its contents.

**Demo source:** `docs/cissp/04-communication-and-network-security/osi-layers.md`.

**Variant A — find the outlier:**
> Pick the Fact from a different Column than the others. (UI: "Outlier mode.")
...
>
> *Engine pulls 3 Facts from one Column + 1 Fact from a different Column (same Row). Student identifies the cross-Column one. Tests Column literacy.*
...
**Variant C — same Fact, different prompt shape:**
> Equivalence check (UI: "Are these the same Fact?"):
> Q1: `Data Link | PDU → ?` (correct: Frame)
> Q2: `? | PDU → Frame` (correct: Data Link)
> Q3: `Frame ∈ ? | PDU` (correct: Data Link)
>
> *All three resolve to the same Fact `(osi-layers, Data Link, PDU, Frame)`. Tests whether the student grasps the Fact, not the prompt shape.*

---

## 7. Pedagogical moves — only possible with traced Facts
...
Each hint is itself a coordinate Fact pulled from the answer Row. No English description, no LLM. Costs a hint point.
...
**Easier-version-on-failure.** Miss `Network | key devices → ?` with all-Column distractors? Engine retries the *same Fact* with cross-Column distractors (easier pool). Then once the student gets it, ramps back up. Today's wrong becomes tomorrow's easy correct.

**Hot streak surrender.** Student has nailed `Physical | PDU → Bit` five different ways (MC, fill-in, T/F, hide row, hide column). Engine suspends the Fact from active rotation. Spaced repetition reintroduces it weeks later — long enough to test retention, short enough that a real loss is recoverable.
...
|------|----------------------|
| **Drill** | One Concept, all its Facts, every axis/format combination. Used when the student is targeting a specific area. |
...
| **Spaced** | The daily review queue. Due Facts (re-test scheduled) interleaved with new Facts (never seen). |
...
| **Random Walk** | Engine picks the next question based on what would be most diagnostic *right now* — Facts with the largest uncertainty in the model. |
| **Cram** | Every Fact the student has ever missed, ordered by recency × failure count. The night-before-the-exam mode. |

Modes are configuration over the same generator — the engine doesn't have separate code paths per mode, just different Fact-selection policies.
...
- **"This Concept can generate N distinct questions."** Live counter as you author. A 3-row × 4-column Dimensions Concept with single-Fact cells: ~36 distinct questions. Add multi-Fact cells, you're well into the hundreds. Authors immediately know if a Concept is too thin.

- **Distractor coverage report.** "Every Fact in this Concept has been used as a distractor at least M times across the dataset." Identifies dead Facts that nothing tests *against*.

- **Atomicity linter.** Author types `Define scope, roles, and resources` into a cell? Engine flags before save: "Comma-list / 'and'-joined Facts detected. Split into 3 with `<br>`?" Must be enforced at author time — expensive to re-atomize later.
