...
- **All six are Possession factor.** TOTP, HOTP, SMS, push, FIDO2, smart card all sit in the Possession category. Wrong-answer pattern: claiming TOTP is "something you know" because the user types a code. The code is generated by something the user *has*.
- **SMS is the weakest practical MFA.** SIM swapping and SS7 attacks defeat SMS without touching the user's device. NIST SP 800-63B explicitly *restricts* SMS as an out-of-band authenticator. Wrong-answer pattern: recommending SMS as "good enough" for high-value accounts.
- **TOTP/HOTP are not phishing-resistant.** OTP codes can be relayed in real time via reverse proxy (Evilginx). Wrong-answer pattern: claiming TOTP defeats phishing — it doesn't.
- **FIDO2 phishing-resistance.** Origin binding via the WebAuthn ceremony makes FIDO2 phishing-resistant. The authenticator refuses to sign for a look-alike domain. Wrong-answer pattern: claiming FIDO2 phishing-resistance comes from "biometric proof" — it comes from origin binding.
- **Push fatigue.** Attackers trigger repeated push notifications until the victim approves to stop the noise. Mitigation: number-matching. Wrong-answer pattern: claiming push notifications are inherently safer than OTP.
- **HOTP vs TOTP.** HOTP uses a counter; TOTP uses time. Both are typically 6-digit codes. Wrong-answer pattern: confusing the moving factor — counter (HOTP) vs time (TOTP).
